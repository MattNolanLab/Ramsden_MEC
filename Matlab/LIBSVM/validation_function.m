function ret1 = validation_function(dec, labels)

labels = (labels >= 0) - (labels < 0);
% 
valid_function = @(dec, labels) auc(dec, labels);
ret1 = valid_function(dec, labels);
% valid_function = @(dec, labels) precision(dec, labels);
% ret2 = valid_function(dec, labels);
% valid_function = @(dec, labels) recall(dec, labels);
% ret3 = valid_function(dec, labels);
% valid_function = @(dec, labels) bac(dec, labels);
% ret4 = valid_function(dec, labels);
valid_function = @(dec, labels) accuracy(dec, labels);
ret5 = valid_function(dec, labels);
% valid_function = @(dec, labels) fscore(dec, labels);
% ret6 = valid_function(dec, labels);
